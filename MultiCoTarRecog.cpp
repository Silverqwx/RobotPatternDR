...
bool MultiCoTarRecog::GetLines(cv::Point2d Center, double* CrossLine, int line_length)//画出矩形的4条边，并按照顺时针顺序，从标记点开始输出四个顶点的图像坐标
{
	cv::Point2i centerpixel = Center;
	float length1, length2, length3, length4;
	////////float Points[200];//中心交叉线线头找到的40个点
	float* Points = new float[line_length]();//中心交叉线线头找到的40个点
	...
	//////int VLine[200];
	int* PLine1 = new int[line_length]();//中心交叉线线头找到的40个点
	int* PLine2 = new int[line_length]();//中心交叉线线头找到的40个点
	int* VLine = new int[line_length]();//中心交叉线线头找到的40个点

	LG->RayLineFunction(CrossLine[0], CrossLine[1], line_length_point_num, PLine1);//生成前进搜索模板,CrossLine是由霍夫变换得到的中心线方程
	LG->RayLineFunction(CrossLine[3], CrossLine[4], line_length_point_num, PLine2);//生成垂直模板
